...
void WINS_GetLocalAddress()
{
    ...
    addr = ntohl(myAddr);
    my_tcpip_address =
        std::format("{}.{}.{}.{}", (addr >> 24) & 0xff, (addr >> 16) & 0xff, (addr >> 8) & 0xff, addr & 0xff);
}
...
int WINS_GetNameFromAddr(struct qsockaddr *addr, char *name)
{
    ...
    hostentry = gethostbyaddr(reinterpret_cast<char *>(&((struct sockaddr_in *)addr)->sin_addr), sizeof(struct in_addr),
                              AF_INET);
    if (hostentry)
    {
