...
qboolean VID_SetWindowedMode(int modenum)
{
    ...
    dibwindow = CreateWindowEx(ExWindowStyle, "WinQuake", "GLQuake", WindowStyle, rect.left, rect.top, width, height,
                               nullptr, nullptr, global_hInstance, nullptr);
...
static std::string GL_BuildExtensionsString(void)
{
    ...
    PFNGLGETSTRINGIPROC qglGetStringi = reinterpret_cast<PFNGLGETSTRINGIPROC>(wglGetProcAddress("glGetStringi"));
    if (!qglGetStringi)
    {
        return "";
    }
...
void GL_BeginRendering(int *x, int *y, int *width, int *height)
{
    ...
    *height = WindowRect.bottom - WindowRect.top;
    ...
    if (qglBindSampler)
    {
        qglBindSampler(0, 0); // restore texture-object filter for 3D
    }
...
std::optional<std::string> VID_GetModeDescription(int mode)
{
    ...
    return std::format("Desktop resolution ({}x{})", modelist[MODE_FULLSCREEN_DEFAULT].width,
...
void VID_InitDIB(HINSTANCE hInstance)
{
    ...
    modelist[0].modedesc = std::format("{}x{}", modelist[0].width, modelist[0].height);
...
void VID_InitFullDIB(HINSTANCE hInstance)
{
    ...
    do
    {
        ...
        if ((devmode.dmBitsPerPel >= 15) && (devmode.dmPelsWidth <= MAXWIDTH) && (devmode.dmPelsHeight <= MAXHEIGHT) &&
            (nummodes < MAX_MODE_LIST))
        {
            ...
            if (ChangeDisplaySettings(&devmode, CDS_TEST | CDS_FULLSCREEN) == DISP_CHANGE_SUCCESSFUL)
            {
                ...
                modelist[nummodes].modedesc =
                    std::format("{}x{}x{}", devmode.dmPelsWidth, devmode.dmPelsHeight, devmode.dmBitsPerPel);
                ...
                if (!COM_CheckParm("-noadjustaspect"))
                {
                    if (modelist[nummodes].width > (modelist[nummodes].height << 1))
                    {
                        ...
                        modelist[nummodes].modedesc = std::format("{}x{}x{}", modelist[nummodes].width,
                                                                  modelist[nummodes].height, modelist[nummodes].bpp);
                    }
                }
...
static void Check_Gamma(unsigned char *pal)
{
    ...
    {
        if (gl_renderer.find("Voodoo") != std::string::npos || gl_vendor.find("3Dfx") != std::string::npos)
        {
            vid_gamma = 1;
        }
        else
...
void VID_Init(unsigned char *palette)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (COM_CheckParm("-current") || (!COM_CheckParm("-width") && !COM_CheckParm("-height")))
            {
    ...
    if (!dummyRC)
    {
        Sys_Error("Could not initialize GL (wglCreateContext failed).\n\nMake sure you in are 65535 color mode, and "
                  "try running -window.");
    }
    ...
    {
        int attribs[] = {
            WGL_CONTEXT_MAJOR_VERSION_ARB,    4, WGL_CONTEXT_MINOR_VERSION_ARB, 5, WGL_CONTEXT_PROFILE_MASK_ARB,
