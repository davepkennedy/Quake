...
void SV_PushRotate(edict_t *pusher, float movetime)
{
    ...
    for (e = 1; e < SV_NumEdicts(); e++, check = NEXT_EDICT(check))
    {
        ...
        if (!(((int)check->v.flags & FL_ONGROUND) && PROG_TO_EDICT(check->v.groundentity) == pusher))
        {
            if (check->v.absmin[0] >= pusher->v.absmax[0] || check->v.absmin[1] >= pusher->v.absmax[1] ||
                check->v.absmin[2] >= pusher->v.absmax[2] || check->v.absmax[0] <= pusher->v.absmin[0] ||
                check->v.absmax[1] <= pusher->v.absmin[1] || check->v.absmax[2] <= pusher->v.absmin[2])
            {
                continue;
            }
...
qboolean SV_CheckWater(edict_t *ent)
{
    ...
    if (cont <= CONTENTS_WATER)
    {
        ...
        if (truecont <= CONTENTS_CURRENT_0 && truecont >= CONTENTS_CURRENT_DOWN)
        {
            ...
            VectorMA(ent->v.basevelocity, 150.0 * ent->v.waterlevel / 3.0, current_table[CONTENTS_CURRENT_0 - truecont],
                     ent->v.basevelocity);
        }
#endif
    }
...
void SV_Physics_Toss(edict_t *ent)
{
    ...
    if (!((int)ent->v.flags & FL_ONGROUND) && ent->v.movetype != MOVETYPE_FLY &&
        ent->v.movetype != MOVETYPE_BOUNCEMISSILE && ent->v.movetype != MOVETYPE_FLYMISSILE)
    {
        SV_AddGravity(ent);
    }
