...
static void Draw2D_Quad(float x, float y, float w, float h, float sl, float tl, float sh, float th)
{
    float v[24] = {
        x, y, sl, tl, x + w, y, sh, tl, x + w, y + h, sh, th, x, y, sl, tl, x + w, y + h, sh, th, x, y + h, sl, th,
    };
...
void Draw_Init(void)
{
    ...
    if (!Q_strncasecmp(gl_renderer.c_str(), "3dfx", 4) || gl_renderer.find("Glide") != std::string::npos)
    {
...
void Draw_AlphaPic(int x, int y, qpic_t *pic, float alpha)
{
    ...
    Draw2D_Quad((float)x, (float)y, (float)pic->width, (float)pic->height, gl->sl, gl->tl, gl->sh, gl->th);
    glDisable(GL_BLEND);
...
void Draw_TransPicTranslate(int x, int y, qpic_t *pic, byte *translation)
{
    ...
    const float white[4] = {1.f, 1.f, 1.f, 1.f};
    qglUniform4fv(u_color_loc, 1, white);
    Draw2D_Quad((float)x, (float)y, (float)pic->width, (float)pic->height, 0.f, 0.f, 1.f, 1.f);
}
...
void Draw_TileClear(int x, int y, int w, int h)
{
    ...
    const float white[4] = {1.f, 1.f, 1.f, 1.f};
    qglUniform4fv(u_color_loc, 1, white);
    Draw2D_Quad((float)x, (float)y, (float)w, (float)h, x / 64.f, y / 64.f, (x + w) / 64.f, (y + h) / 64.f);
}
...
void Draw_Fill(int x, int y, int w, int h, int c)
{
    ...
    const float color[4] = {host_basepal[c * 3] / 255.f, host_basepal[c * 3 + 1] / 255.f,
                            host_basepal[c * 3 + 2] / 255.f, 1.f};
    qglUniform4fv(u_color_loc, 1, color);
